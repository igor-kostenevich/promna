...
>(formState: T | Ref<T>, rules: R, options?: UseValidationOptions) {
  ...
  const isFormValid = computed(() => {
    const v = v$.value;
    const form = toValue(formState);
    const rulesObj = toValue(rules);

    const requiredFields = Object.keys(rulesObj);
    if (!requiredFields.length) return false;

    const anyDirty = requiredFields.some((key) => v[key]?.$dirty);
    if (!anyDirty) return false;

    const anyInvalidOrEmpty = requiredFields.some((key) => {
